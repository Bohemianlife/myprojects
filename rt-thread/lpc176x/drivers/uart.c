...
/*@{*/
#if (defined(RT_USING_UART0) || defined(RT_USING_UART3)) && defined(RT_USING_DEVICE)
...
/***定义了一个结构名为rt_uart_lpc的结构变量uart_device***/
struct rt_uart_lpc
{
	...
	/* 
	 * read_index表示已经读取(即已被处理)的索引
	 * save_index表示下一个可以用于存储接受数据的索引
	 */
...
void UART3_IRQHandler(void)
{
	rt_ubase_t level, iir;
    struct rt_uart_lpc* uart = &uart_device;
/**struct rt_uart_lpc* uart;定义指向结构体类型的指针变量uart;**/
    /* read IIR and clear it */
	iir = LPC_UART->IIR;
...
//	if (LPC_UART == LPC_UART0)
	if ((LPC_UART_TypeDef *)LPC_UART == LPC_UART3)
	{
 ...
 * 理解:dev 是一个指针，指向rt_device的结构体的指针，强制类型转换之后和uart指针是等价的。
 *      也就是说uart指向dev
 * 原因：强制转换就是为了访问到rx_buffer[uart->read_index]。
 * uart = (struct stm32_serial_device*) dev->user_data;
 * 理解:user_data是dev的一个成员变量，也是一个指针，指向一块内存，user_data强制转换之后和
 *      uart指针是等价的。
 * 原因：强制转换就是为了让user_data访问到uart.
 */
	struct rt_uart_lpc *uart = (struct rt_uart_lpc*)dev;//(struct rt_uart_lpc*)dev是强制类型转换
	RT_ASSERT(uart != RT_NULL);

	/* point to buffer */
	ptr = (rt_uint8_t*) buffer;//(rt_uint8_t*) buffer是强制类型转换
	if (dev->flag & RT_DEVICE_FLAG_INT_RX)
	{
		while (size)
		{
			...
			if (uart->read_index != uart->save_index)//如果有数据可读
			{
